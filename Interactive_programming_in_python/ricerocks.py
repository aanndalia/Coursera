...
class Sprite:
    ...
    def update(self):
        ...
        # update age and check if exceeds lifespan
        self.age += 1
...
def draw(canvas):
    ...
    rocks_shot = group_group_collide(missile_group, rock_group)
    score += rocks_shot
...
def process_sprite_group(canvas, sprite_group):
    remove_set = set([])
    sprite_group_set = set(sprite_group)
    for s in sprite_group_set:
        s.update()
        shouldRemove = s.update()
        if shouldRemove:
            remove_set.add(s)
        s.draw(canvas)
...
def rock_spawner():
    ...
    if (len(rock_group) < MAX_ROCKS) and started:            
        ...
        rock_vel_range = score*0.2 + .6  
        rock_vel = [random.random() * rock_vel_range - rock_vel_range/2, random.random() * rock_vel_range - rock_vel_range/2]
