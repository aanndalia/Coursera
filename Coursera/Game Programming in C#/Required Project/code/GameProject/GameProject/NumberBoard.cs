    ...
    class NumberBoard
    {
            ...
            int correctNumber, SoundBank soundBank)
        {
            ...
            for (int row = 0; row < NUM_ROWS; ++row)
            {
                ...
                {
                    int tileNum = 3 * row + col + 1;
                    tiles[row, col] = new NumberTile(contentManager, CalculateTileCenter(row, col), tileSideLength, tileNum, correctNumber, soundBank);
                }
            }
        ...
        public bool Update(GameTime gameTime, MouseState mouse)
        {
            ...
            for (int row = 0; row < NUM_ROWS; ++row)
            {
                for (int col = 0; col < NUM_COLUMNS; ++col)
                {
                    bool correctGuess = tiles[row, col].Update(gameTime, mouse);

                    // return true if one of the number tiles returned that it was selected as the correct guess
                    if (correctGuess)
                        return true;
                }
            }
